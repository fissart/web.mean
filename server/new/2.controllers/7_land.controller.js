...
async function getsController(req, res) {
    const data = await _7_Collection_1.default.find({ type: 'lan' });
...
async function updateController(req, res) {
    ...
    if (req.file) {
        ...
        const updatedCurse = await _7_Collection_1.default.findByIdAndUpdate(id, { title, description, responce, foto: req.file.path });
    }
    else {
        const updatedCurse = await _7_Collection_1.default.findByIdAndUpdate(id, { title, description, responce });
    }
    return res.json({
